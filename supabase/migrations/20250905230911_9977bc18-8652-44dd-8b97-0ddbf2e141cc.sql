-- Create notifications table with proper schema and RLS (handling existing type)
DO $$ 
BEGIN
    -- Check if the type exists and create if not
...
DROP TRIGGER IF EXISTS appointments_notification_trigger ON public.appointments;
DROP TRIGGER IF EXISTS cases_notification_trigger ON public.cases;
DROP TRIGGER IF EXISTS documents_notification_trigger ON public.documents;
DROP TRIGGER IF EXISTS messages_notification_trigger ON public.contact_messages;
